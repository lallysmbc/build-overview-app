...
class ProgressCircle extends Component {
    ...
    componentDidMount() {
        this.perCirc(this.props.percentage)
    }
    ...
    perCirc(end, i) {
        ...
        if (i <= 180) {
            ...
            this.perCircContainer.current.style.backgroundImage = 'linear-gradient(' + (i - 90) + 'deg, transparent 50%, #00cc00 50%),linear-gradient(90deg, #F7F9FA 50%, transparent 50%)';
        }
        if (curr < end) {
            setTimeout(() => 
                this.perCirc(end, ++i), 1);
        }
    }
